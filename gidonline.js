...
(function () {
		 ...
		 network["native"](url, function (str) {
			...
			if (videos) {
			  var video = decode(videos[1]),
					qused = '',
					first = '',
					mass = ['2160p', '1440p', '1080p Ultra', '1080p', '720p', '480p', '360p']; //ÑƒÑ…Ð½Ñ Ñ‚ÑƒÑ‚ Ð¿Ñ€Ð¾Ð¸ÑÑ…Ð¾Ð´Ð¸Ñ‚, Ñ…Ñ€ÐµÐ½ Ð·Ð½Ð°ÐµÑ‚ Ð¿Ð¾Ñ‡ÐµÐ¼Ñƒ Ð¿Ð¾ÑÐ»Ðµ .join() Ð²Ð¾Ð·Ð²Ñ€Ð¾ÑˆÐ°ÐµÑ‚ Ñ‚Ð¾Ð»ÑŒÐºÐ¾ Ð¿Ð¾ÑÐ»ÐµÐ´Ð½Ð¸ÑŽ ÑÑÑ‹Ð»ÐºÑƒ
	...
	function kinobase(component, _object) {
	  ...
	  function extractData(str, page) {
		 ...
		 if (vod[0] == 'file') {
			...
			var quality_type = page.replace(/\n/g, '').replace(/ /g, '').match(/<li><b>ÐšÐ°Ñ‡ÐµÑÑ‚Ð²Ð¾:<\/b>(\w+)<\/li>/i);
 ...
 if(window.filmix.max_qualitie == 720) window.filmix.max_qualitie = 480
	...
	function component(object) {
	  ...
	  var kiposk_sources = ['rezka', 'collaps']; // ÑˆÐ°Ð»Ð¾Ð²Ð»Ð¸Ð²Ñ‹Ðµ Ñ€ÑƒÑ‡ÐºÐ¸
	  ...
	  /**
		* ÐŸÐ¾ÐºÐ°Ð·Ð°Ñ‚ÑŒ Ð¿ÑƒÑÑ‚Ð¾Ð¹ Ñ€ÐµÐ·ÑƒÐ»ÑŒÑ‚Ð°Ñ‚ Ð¿Ð¾ ÐºÐ»ÑŽÑ‡ÐµÐ²Ð¾Ð¼Ñƒ ÑÐ»Ð¾Ð²Ñƒ
		*/
	  ...
	  this.start = function (first_select) {
		 if (Lampa.Activity.active().activity !== this.activity) return; //Ð¾Ð±ÑÐ·Ð°Ñ‚ÐµÐ»ÑŒÐ½Ð¾, Ð¸Ð½Ð°Ñ‡Ðµ Ð½Ð°Ð±Ð»ÑŽÐ´Ð°ÐµÑ‚ÑÑ Ð±Ð°Ð³, Ð°ÐºÑ‚Ð¸Ð²Ð½Ð¾ÑÑ‚ÑŒ ÑÐ¾Ð·Ð´Ð°ÐµÑ‚ÑÑ Ð½Ð¾ Ð½Ðµ ÑÑ‚Ð°Ñ€Ñ‚ÑƒÐµÑ‚, Ð² Ñ‚Ð¾ Ð²Ñ€ÐµÐ¼Ñ ÐºÐ°Ðº ÐºÐ¾Ð¼Ð¿Ð¾Ð½ÐµÐ½Ñ‚ Ð·Ð°Ð³Ñ€ÑƒÐ¶Ð°ÐµÑ‚ÑÑ Ð¸ ÑÑ‚Ð°Ñ€Ñ‚ÑƒÐµÑ‚ ÑÐ°Ð¼Ð¾Ð³Ð¾ ÑÐµÐ±Ñ.
